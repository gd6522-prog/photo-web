...
const cargoColumns: Array<{ key: keyof CargoRow | "largeTotal" | "smallTotal" | "support"; label: string; width?: number }> = [
  ...
  { key: "car_no", label: "호차", width: 58 },
  { key: "seq_no", label: "순번", width: 50 },
  { key: "store_code", label: "점포코드", width: 80 },
  { key: "store_name", label: "점포명", width: 230 },
  { key: "largeTotal", label: "대", width: 72 },
  { key: "large_box", label: "박스존", width: 64 },
  { key: "large_inner", label: "이너존", width: 64 },
  { key: "large_other", label: "기타", width: 64 },
  { key: "large_day2l", label: "올데이 2L생수", width: 88 },
  { key: "large_nb2l", label: "노브랜드2L생수", width: 88 },
  { key: "smallTotal", label: "소", width: 72 },
  { key: "small_low", label: "경량존", width: 64 },
  { key: "small_high", label: "슬라존", width: 64 },
  { key: "event", label: "행사", width: 64 },
  { key: "tobacco", label: "담배", width: 68 },
  { key: "certificate", label: "유가증권", width: 80 },
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", width: "100%" }}>
      <input
        ...
        style={{ width: "100%", minWidth: 100, height: 34, borderRadius: 0, border: "1px solid #d6e4ee", padding: "0 6px", outline: "none", boxSizing: "border-box", fontSize: 12 }}
      />
                  ...
                  {[...group.rows, ...Array.from({ length: Math.max(0, 20 - group.rows.length) }, (_, i) => ({ id: `blank-${group.carNo}-${i}` } as CargoRow))].slice(0, 20).map((row, index) => {
                    ...
                    return (
                      <tr key={row.id || `${group.carNo}-${index}`} style={{ background: reportRowBackground, height: REPORT_BODY_ROW_HEIGHT, minHeight: REPORT_BODY_ROW_HEIGHT, maxHeight: REPORT_BODY_ROW_HEIGHT, lineHeight: 1, ...reportRowPca }}>
                        ...
                        <td style={{ border: "1px solid #666", height: REPORT_BODY_ROW_HEIGHT, minHeight: REPORT_BODY_ROW_HEIGHT, maxHeight: REPORT_BODY_ROW_HEIGHT, padding: "6px 5px", textAlign: "center", verticalAlign: "middle", overflow: "hidden", color: (!supportMode && row.support_excluded) ? "#fff" : "#b91c1c", background: reportRowBackground, fontWeight: 900, fontSize: REPORT_WATER_NUMBER_FONT_SIZE, ...getReportSectionStyle(6), ...getReportMainCellWidth(6) }}>{row.large_day2l ? formatNumber(row.large_day2l) : ""}</td>
...
export default function VehiclePage() {
  return <VehiclePageScreen initialTab="cargo" allowedTabs={["cargo"]} title="물동량" />;
}
