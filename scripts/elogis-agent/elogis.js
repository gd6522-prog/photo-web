  ...
  for (const frame of frames) {
    ...
    try {
      probe = await frame.evaluate(() => {
        if (typeof Ext === "undefined") return { extOk: false };
        const all = Ext.ComponentQuery.query("numberfield");
        const visible = all.filter((nf) => nf.isVisible && nf.isVisible(true));
        const matched = visible.filter((nf) => /grid\w+PageSize/i.test(nf.name || "") && !/CurrentPage/i.test(nf.name || ""));
        return {
          extOk: true,
          total: all.length,
          visible: visible.length,
          matched: matched.length,
          probe: matched.map((nf) => ({
            extName: nf.name,
            inputName: nf.inputEl?.dom?.name || null,
            inputId: nf.inputEl?.dom?.id || null,
          })),
        };
      });
    } catch (e) { probe = { error: String(e?.message ?? e) }; }
    const url = (frame.url ? frame.url() : "main").slice(-60);
    log(`${label}: [DEBUG] frame ...${url} → ${JSON.stringify(probe).slice(0, 400)}`);
    const p = probe?.probe?.[0];
    if (!p) continue;
    // selector 결정 — id 가 있으면 id 우선, 없으면 name
    let selector = null;
